...
### Defining Scenario Function (modelled by the logistic regression)
...
iter<-5000 # Number of Samples Used by MCMC 

# The size of the next phase 2 used to assess the predictive properties of the design
testing.set<-112

# Baclofen doses (escalated compound)
...
futility<-0.925                  # Probability controlling early stopping for ``all combinations are safe''


# Apply safety/futility stopping
safety.stopping<-T
futility.stopping<-T

# Parameter of the Normal distribution of Methadone Doses
mean.methadone<-51.4
sd.methadone<-23.3
min.methadone<-10

# Combinations for safety/futility constraints
...
# This is calibrated operational prior as per the manuscript
...
# Running Simulations
  for (z in 1:nsims){
    ...
    # If not stopped earlier, predict for the next Phase
    if(stop==0){
